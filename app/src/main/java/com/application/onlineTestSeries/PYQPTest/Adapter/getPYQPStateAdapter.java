...
public class getPYQPStateAdapter extends RecyclerView.Adapter<getPYQPStateAdapter.MyViewHolder> implements Filterable {
    public List<State> cityList;
    private Context mContext;
    private List<State> filterList;
    private StateAdapterListener listener;
    public getPYQPStateAdapter(Context mContext, List<State> cityList, StateAdapterListener listener) {
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        State cityState = filterList.get(position);
        holder.mCardView.setTag(position);
        holder.stateName.setText(cityState.getName());
    }
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            protected void publishResults(CharSequence charSequence, FilterResults filterResults) {
                filterList = (ArrayList<State>) filterResults.values;
                notifyDataSetChanged();
            }
        };
    }
    public interface StateAdapterListener {
        void onStateSelected(State students);
    }
